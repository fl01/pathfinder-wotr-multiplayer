...
namespace WOTRMultiplayer.HarmonyPatches.ContextActions
{
    ...
    public class ContextActionOnRandomTargetsAroundPatches
    {
        ...
        public static IEnumerable<CodeInstruction> ContextActionOnRandomTargetsAround_RunAction_Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            Main.GetLogger<ContextActionOnRandomTargetsAroundPatches>().LogDebug("Transpiler has been applied. Target={Target}", target);
            return matcher.Instructions();
        }
        ...
        private static UnitEntityData SelectRandomTarget(IList<UnitEntityData> targets, ContextActionOnRandomTargetsAround contextActionOnRandomTargetsAround)
        {
            ...
            try
            {
                ...
                var identifier = $"{nameof(ContextActionOnRandomTargetsAround)}:{nameof(SelectRandomTarget)}:{units}:{unitId}:{targetId}:{abilityName}:{attackNumber}_{seededContext.Id}";
                var index = Main.Multiplayer.ValueGenerator.Range(seededContext.Lifetime, identifier, 0, targets.Count);
                var unit = targets[index];
                Main.GetLogger<ContextActionRandomizePatches>().LogInformation("ContextActionOnRandomTargetsAround has been rolled. UnitId={UnitId}, Identifier={Identifier}", unitId, identifier);
                return unit;
            }
            catch (Exception ex)
            {
